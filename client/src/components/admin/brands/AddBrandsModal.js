...
import { createBrands, getAllBrands } from "./FetchApi";

const AddBrandsModal = (props) => {
  ...
  const submitForm = async (e) => {
    ...
    try {
      let responseData = await createBrands(fData);
  ...
  return (
    <Fragment>
      {/* Black Overlay */}
      <div
        onClick={(e) => dispatch({ type: "addBrandsModal", payload: false })}
        className={`${
          data.addBrandsModal ? "" : "hidden"
        ...
        <div className="relative bg-white w-12/12 md:w-3/6 shadow-lg flex flex-col items-center space-y-4  overflow-y-auto px-4 py-4 md:px-8">
          <div className="flex items-center justify-between w-full pt-4">
            ...
            <span
              ...
              onClick={(e) =>
                dispatch({ type: "addBrandsModal", payload: false })
              }
          ...
          <form className="w-full" onSubmit={(e) => submitForm(e)}>
            ...
            <div className="flex flex-col space-y-1 w-full">
              <label htmlFor="name">Brand Image</label>
              <input
                accept=".jpg, .jpeg, .png"
                onChange={(e) => {
                  setFdata({
