...
import { getAllBrands, deleteBrands } from "./FetchApi";
import { BrandsContext } from "./index";
import moment from "moment";

const apiURL = process.env.REACT_APP_API_URL;

const AllBrands = (props) => {
  const { data, dispatch } = useContext(BrandsContext);
  ...
  const fetchData = async () => {
    ...
    setTimeout(() => {
      if (responseData && responseData.brands) {
        dispatch({
          type: "fetchBrandsAndChangeState",
  ...
  const deleteBrandsReq = async (cId) => {
    let deleteC = await deleteBrands(cId);
    if (deleteC.error) {
  ...
  const editBrands = (cId, type, des, status) => {
    if (type) {
      dispatch({
        type: "editBrandsModalOpen",
        cId: cId,
        des: des,
        status: status,
      });
    ...
    <Fragment>
      <div className="col-span-1 overflow-auto bg-white shadow-lg p-4">
                ...
                return (
                  <BrandsTable
                    ...
                    editCat={(cId, type, des, status) =>
                      editBrands(cId, type, des, status)
                    }
                    deleteCat={(cId) => deleteBrandsReq(cId)}
                    key={key}
                  />
                );
              })
        ...
        <div className="text-sm text-gray-600 mt-2">
          Total {categories && categories.length} brands found
        </div>
      </div>
    </Fragment>
  );
...
const BrandsTable = ({ category, deleteCat, editCat }) => {
  return (
    <Fragment>
      <tr>
